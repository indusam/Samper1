...
class AccountEdiFormat(models.Model):
    """Extend account.edi.format to add detallista namespace for Liverpool addenda."""
    ...
    def _l10n_mx_edi_export_invoice_cfdi(self, invoice):
        ...
        Returns:
            dict: CFDI result with 'cfdi_str' and 'errors'
        """
        # Get the CFDI from parent method
        res = super()._l10n_mx_edi_export_invoice_cfdi(invoice)

        _logger.info(
            'Export CFDI for invoice %s, require_addenda_liverpool: %s',
            invoice.name, invoice.require_addenda_liverpool
        )
        ...
        if invoice.require_addenda_liverpool and res.get('cfdi_str'):
            try:
                _logger.info(
                    'Adding detallista namespace to CFDI for invoice %s',
                    invoice.name
                )
                ...
                if 'detallista' not in current_schema:
                    ...
                    _logger.info(
                        'New schema location: %s',
                        cfdi_node.get(schema_loc_key)
                    )

                # Convert back to bytes - PAC expects bytes, not string
                res['cfdi_str'] = etree.tostring(
                    cfdi_node, encoding='utf-8', pretty_print=False
                )
                _logger.info('CFDI updated successfully')

            except Exception as e:
                _logger.error(
                    'Error adding detallista namespace to CFDI for invoice %s: %s',
