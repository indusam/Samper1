...
class Formulas(models.TransientModel):
    ...
    def get_cod_prov(self, product_tmpl_id):
        supplier_info = self.env['product.supplierinfo'].search([
            ('product_tmpl_id', '=', product_tmpl_id)], limit=1)
        return supplier_info.product_name if supplier_info else ''

    def get_norden(self, default_code):
        if 'ca' in default_code:
            return '1 Cárnicos'
        elif 'ad' in default_code:
            return '2 Aditivos'
        elif 'in' in default_code:
            return '3 Intermedios'
        else:
            return '4 '
    ...
    def consolidate_component(self, componente, ncant_limitante):
        ...
        if not ncomponente:
            codprov = self.get_cod_prov(componente.product_id.product_tmpl_id.id)
            norden = self.get_norden(componente.product_id.default_code)
        ...
        for ingrediente in ingredientes:
            if ingrediente.product_id.bom_count > 0:
                bom_pf = self.env['mrp.bom'].search([
                    ('product_tmpl_id', '=', ingrediente.product_tmpl_id.id)],
                    limit=1).id
                ...
                for componente_n1 in subformula_n1:
                    if componente_n1.product_id.bom_count > 0:
                        bom_pf2 = self.env['mrp.bom'].search([
                            ('product_tmpl_id', '=', componente_n1.product_tmpl_id.id)],
                            limit=1).id

                        subformula_n2 = self.env['mrp.bom.line'].search([
                            ('bom_id.id', '=', bom_pf2)])

                        for componente_n2 in subformula_n2:
                            if componente_n2.product_id.bom_count > 0:
                                bom_pf3 = self.env['mrp.bom'].search([
                                    ('product_tmpl_id', '=', componente_n2.product_tmpl_id.id)],
                                    limit=1).id

                                subformula_n3 = self.env['mrp.bom.line'].search([
                                    ('bom_id.id', '=', bom_pf3)])
    ...
    def imprime_formula(self):
        ...
        if not self.ing_limitante:
            ...
            for ingrediente in ingredientes:
                codprov = self.get_cod_prov(ingrediente.product_id.product_tmpl_id.id)
                norden = self.get_norden(ingrediente.product_id.default_code)
