...
class Formulas(models.TransientModel):
    ...
    def imprime_formula(self):
        vals = []
        ingredientes = self.env['mrp.bom.line'].search([('bom_id.id', '=', self.producto.id)])

        if not self.ing_limitante:
            for ingrediente in ingredientes:
                codprov = self.env['product.supplierinfo'].search([
                    ('product_tmpl_id', '=', ingrediente.product_id.product_tmpl_id.id)], limit=1
                ).product_name

                norden = {'ca': '1 Cárnicos', 'ad': '2 Aditivos', 'in': '3 Intermedios'}.get(
                    ingrediente.product_id.default_code[:2], '4 '
                )

                vals.append({
                    'componente': ingrediente.product_id.name,
                    'cod_prov': codprov,
                    'cant_comp': ingrediente.product_qty * self.partidas if self.partidas > 0 else self.cantidad * (ingrediente.x_porcentaje / 100),
        ...
        if self.consolidado:
            ...
            if self.ing_limitante:
                ntotcantidad = sum(self.cant_limitante * (ingrediente.product_qty / self.ing_limitante.product_qty) for ingrediente in ingredientes)
            ...
            for ingrediente in ingredientes:
                if ingrediente.product_id.bom_count > 0:
                    ...
                    for componente_n1 in subformula_n1:
                        ncomponente_n1 = self.env['wizard.formulas'].search([('ingr.id','=', componente_n1.product_id.id),('x_secuencia','=',nsecuencia)])

                        if not ncomponente_n1:
                            codprov = self.env['product.supplierinfo'].search([('product_tmpl_id.id', '=', componente_n1.product_id.product_tmpl_id.id)], limit=1).product_name

                            norden = {'ca': '1 Cárnicos', 'ad': '2 Aditivos', 'in': '3 Intermedios'}.get(
                                componente_n1.product_id.default_code[:2], '4 '
                            )
                    ...
                    ncant_limitante = self.cantidad * (ingrediente.x_porcentaje / 100)
                    ncomponente = self.env['wizard.formulas'].search([('ingr.id', '=', ingrediente.product_id.id),('x_secuencia', '=', nsecuencia)])
