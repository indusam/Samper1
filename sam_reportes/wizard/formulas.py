...
class Formulas(models.TransientModel):
    ...
    def _consolidar_formula_recursiva(self, ingrediente, cantidad, nivel):
        if nivel > 10:
            return
        ...
        for ingrediente in ingredientes:
            ncant_limitante = cantidad * (ingrediente.x_porcentaje / 100)
            ...
            consolidado_existente = self.env['wizard.formulas'].search([
                ('x_secuencia', '=', nsecuencia),
                ('ingr', '=', ingrediente.product_id.id)
            ], limit=1)

            if consolidado_existente:
                ncant_tot = consolidado_existente.cant_tot + ncant_limitante
                consolidado_existente.write({'cant_tot': ncant_tot})
            else:
            ...
            self._consolidar_formula_recursiva(ingrediente, ncant_limitante, nivel + 1)
