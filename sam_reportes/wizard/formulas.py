...
class Formulas(models.TransientModel):
    ...
    def consolidate_recursive(self, ingredientes, nsecuencia, ncantidad, norden):
        vals = []
        for ingrediente in ingredientes:
            if self.ing_limitante:
                ...
                for ing in ingredientes:
                    ntotcantidad += self.cant_limitante * (ing.product_qty / ncantidad_il)
            ...
            if ingrediente.product_id.bom_count > 0:
                ...
                subingredientes = self.env['mrp.bom.line'].search([('bom_id.id', '=', bom_pf)])
                vals += self.consolidate_recursive(subingredientes, nsecuencia, ncant_limitante, norden)

            else:
                ncomponente = self.env['wizard.formulas'].search(
                    [('ingr.id', '=', ingrediente.product_id.id), ('x_secuencia', '=', nsecuencia)])
    ...
    def imprime_formula(self):
        ...
        if self.consolidado:
            ...
            if self.ing_limitante:
                ...
                ncantidad_il = self.ing_limitante.product_qty
                for ingrediente in ingredientes:
                    ntotcantidad += self.cant_limitante * (ingrediente.product_qty / ncantidad_il)
                self.cantidad = ntotcantidad

            vals += self.consolidate_recursive(ingredientes, nsecuencia, self.cantidad, '1')
