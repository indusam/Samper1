...
class Formulas(models.TransientModel):
    ...
    cantidad = fields.Float(string="Cantidad", digits=(16, 6))
    ing_limitante = fields.Many2one('mrp.bom.line',string="Ingrediente limitante")
    cant_limitante = fields.Float(string="Cantidad limitante", digits=(16, 6))
    ...
    cant_tot = fields.Float(string="Cant Total", digits=(16, 6))
    unidad = fields.Char(string="Unidad")
    pct_formula = fields.Float(string="% Fórmula", digits=(16, 6))
    pct_categoria = fields.Float(string="% Grupo", digits=(16, 6))
    pct_merma = fields.Float(string="% Merma", digits=(16, 6))
    ...
    def crear_ncomponente(self, ingrediente, secuencia, ncant_limitante):
        ...
        if not ncomponente:
            ...
            self.env['wizard.formulas'].create({
                ...
                'cod_prov': codprov,
                'cant_tot': float_round(ncant_limitante, precision_digits=6),
                'unidad': ingrediente.product_id.uom_id.name,
                'pct_formula': float_round(ingrediente.x_porcentaje, precision_digits=6),
                'pct_categoria': float_round(ingrediente.x_porcentaje_categoria, precision_digits=6),
                'x_orden': norden
            })
        ...
        if ncomponente:
            ...
            ncant_tot = float_round(ncant + nccomp, precision_digits=6)
            ncomponente.write({'cant_tot': ncant_tot})
            

    def consolida_formula(self, ingredientes, nqty, secuencia):
        for ingrediente in ingredientes:
            ncant_limitante = float_round(nqty * (ingrediente.x_porcentaje / 100), precision_digits=6)
        ...
        if not self.ing_limitante:
            ...
            for ingrediente in ingredientes:
                ...
                vals.append({
                    ...
                    'cod_prov': codprov,
                    'cant_comp': float_round(ingrediente.product_qty * self.partidas, precision_digits=6) if self.partidas > 0 
                                else float_round(self.cantidad * (ingrediente.x_porcentaje / 100), precision_digits=6),
                    'unidad': ingrediente.product_id.uom_id.name,
        ...
        if self.ing_limitante:
            ...
            for ingrediente in ingredientes:
                ...
                vals.append({
                        'componente': ingrediente.product_id.name,
                        'cod_prov': codprov,
                        'cant_comp': float_round(self.cant_limitante * (ingrediente.product_qty / ncantidad_il), precision_digits=6),
                        'unidad': ingrediente.product_id.uom_id.name,
                        'pct_formula': ingrediente.x_porcentaje,
                        'pct_categoria': ingrediente.x_porcentaje_categoria,
                        'orden': norden
            ...
            if self.ing_limitante:
                ...
                for ingrediente in ingredientes:
                    ntotcantidad = float_round(ntotcantidad + self.cant_limitante * (
                                ingrediente.product_qty / ncantidad_il), precision_digits=6)
            ...
            for ingrediente in bom_ordenada1:
                ...
                if ingrediente.cant_tot > 0:
                    vals.append({
                        ...
                        'cant_comp': float_round(ingrediente.cant_tot, precision_digits=6),
                        'unidad': ingrediente.ingr.uom_id.name,
                        'pct_formula': float_round((ingrediente.cant_tot / self.cantidad) * 100, precision_digits=6),
