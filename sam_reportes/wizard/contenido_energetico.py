...
# contenido_energetico.py
# Impresión de la tabla de contenido energético de un producto.
...
class ContenidoEnergetico(models.TransientModel):

    _name = 'wizard.contenido.energetico'
    _description = 'Contenido Energético'

    producto = fields.Many2one('mrp.bom', string="Producto")
    cantidad = fields.Float(string="Porción (grs.)")
    ...
    def imprime_contenido_energetico(self):
            ...
            if not self.ing_limitante:
                for ingrediente in ingredientes:
                    vals.append({
                        ...
                        'pct_grpoli_sat': ingrediente.product_id.x_pct_grasas_poliinsaturadas,
                        'pct_grmono_sat': ingrediente.product_id.x_pct_grasas_moninsaturadas,
                        ...
                        'grasa_polisat_kg': (ingrediente.product_id.x_pct_grasas_poliinsaturadas / 100) * (self.cantidad * (ingrediente.x_porcentaje / 100)),
                        'grasa_monosat_kg': (ingrediente.product_id.x_pct_grasas_moninsaturadas / 100) * (self.cantidad * (ingrediente.x_porcentaje / 100)),
                        'grasa_trans_kg':ingrediente.product_id.x_mgkg_grasas_trans * 10 * (self.cantidad * (ingrediente.x_porcentaje / 100)) /  1000,
            ...
            if self.ing_limitante:
                ...
                for ingrediente in ingredientes:
                    vals.append({
                        ...
                        'grasa_polisat_kg': (ingrediente.product_id.x_pct_grasas_poliinsaturadas / 100) * self.cant_limitante * (ingrediente.product_qty / ncantidad_il),
                        'grasa_monosat_kg': (ingrediente.product_id.x_pct_grasas_moninsaturadas / 100) * self.cant_limitante * (ingrediente.product_qty / ncantidad_il),
                        'grasa_trans_kg': ingrediente.product_id.x_mgkg_grasas_trans * 10 * self.cant_limitante * (ingrediente.product_qty / ncantidad_il) / 1000,
                    ...
                    for componente in subformula:
                        ...
                        if not ncomponente:
                            ...
                            self.env['wizard.tabla.nutrimental'].create({
                                ...
                                'grasa_monosat_kg': (componente.product_id.x_pct_grasas_moninsaturadas / 100) * (ncantidad_il * (componente.x_porcentaje / 100)),
                                'grasa_trans_kg': (componente.product_id.x_mgkg_grasas_trans / 100) * (ncantidad_il * (componente.x_porcentaje / 100)) / 1000,
                ...
                if subf == 0:
                    ...
                    if not ncomponente:
                        ...
                        self.env['wizard.tabla.nutrimental'].create({
                            ...
                            'grasa_polisat_kg': (ingrediente.product_id.x_pct_grasas_poliinsaturadas / 100) * (ncantidad_il * (ingrediente.x_porcentaje / 100)),
                            'grasa_monosat_kg': (ingrediente.product_id.x_pct_grasas_moninsaturadas / 100) * (ncantidad_il * (ingrediente.x_porcentaje / 100)),
                            'grasa_trans_kg': (ingrediente.product_id.x_mgkg_grasas_trans / 100) * (ncantidad_il * (ingrediente.x_porcentaje / 100)) / 1000,
        ...
        report_action = self.env.ref('sam_reportes.contenido_energetico_reporte').report_action(self, data=data)
